...
import ChampionFrameHexagon from "./ChampionFrame/ChampionFrameHexagon";
import ChampionSearch from "./Searchbar/ChampionSearch";

import "./ChampionHexagon.scss";
        ...
        <ChampionSearch onChange={handleChange} search={search}/>
             ...
             <div className="championContainer" >
                {filteredChampions.length < 1 && (
                    <div className="championItem">
                        <ChampionFrameHexagon name={"no matches found"} /> 
                    </div>
                ) }
                
             {
                 filteredChampions.map(({champion: {name, data}}, idx) => {
                     return(
                            <div key={idx}  className="championItem" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
                            {/* {console.log(name, data.id, idx)} */}
                        
                             {/* {name}  */}
                            <ChampionFrameHexagon name={data.id}/>
                            </div>
                     );
                 })
             }
